...
//           const fileName = filePath.split("/").pop().split("-").slice(1).join("-"); // Remove timestamp
...
//                   <a 
//                     href={`http://localhost:5001${file.fullPath}`} // Adjust backend URL if needed
//                     target="_blank"
...
//   const [newFiles, setNewFiles] = useState([]); // 🆕 Store new files to be uploaded
...
//       // Extract original file names (remove timestamp)
//       if (projectToEdit.files) {
//         const formattedFiles = projectToEdit.files.map((filePath) => {
...
//     setNewFiles((prevFiles) => [...prevFiles, ...selectedFiles]); // 🆕 Append new files
//   };
...
//           {newFiles.length > 0 && (
//             <div className="border rounded p-2 bg-gray-100">
//               <p className="text-gray-600 mb-1">Selected Files:</p>
//               {newFiles.map((file, index) => (
//                 <div key={index} className="border p-2 my-1 bg-white shadow-sm rounded flex justify-between">
//                   <span>{file.name}</span>
//                   <button
//                     type="button"
